...
namespace F_Result.Controllers
{
    public class PlanDebitsF2Controller : Controller
    {
        ...
        [Authorize(Roles = "Administrator, Chief, ProjectManager, Accountant, Financier")]
        [HttpPost]
        public ActionResult LoadPD(int[] filterPrjIDs, int[] filterExpendIDs, int[] filterOrgIDs)
        {
            try
            {
                ...
                List<int> _IDsList = _ads.Select(x => x.PlanDebitF2Id).ToList();
                ...
                List<ArticlesIDs> _articlesList = _ads.GroupBy(x => x.ExpenditureId).
